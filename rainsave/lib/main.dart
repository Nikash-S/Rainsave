...
import 'firebase_options.dart'; // This was auto-generated by the CLI
...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  final String userId = "user_001"; // Hardcoded user ID for the hackathon
...
Future<void> _triggerTrustedContactAlert(String triggerReason) async {
    // 1. Show the UI popup for the demo
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 8),
            Text("Safety Trigger", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ],
        ),
        content: Text(
          "Unusual activity detected: $triggerReason.\n\n"
          "As part of your recovery plan, a secure notification has been sent to your trusted contact.",
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text("Understood", style: TextStyle(color: Color(0xFF2E67A0))),
          )
        ],
      ),
    );

    // 2. ACTUAL BACKEND INTEGRATION: Log the alert to Firestore
    try {
      await _db.collection('alerts').add({
        'userId': userId,
        'trustedContact': 'Warren Buffet',
        'contactPhone': '+447123456789',
        'reason': triggerReason,
        'timestamp': FieldValue.serverTimestamp(),
        'status': 'pending_sms_dispatch' // A cloud function would read this and send the SMS
      });
      print("Backend: Alert successfully logged to database.");
    } catch (e) {
...
void _buyTickets() async {
    ...
    if (qty > 0 && linkedBalance >= (qty * 10)) {
      ...
      if (qty >= 10) {
        // Now this writes to the backend!
        await _triggerTrustedContactAlert("Large ticket purchase (£${qty * 10})");
      }
      ...
      // ACTUAL BACKEND INTEGRATION: Sync new balances to Firestore
      try {
        await _db.collection('users').doc(userId).set({
          'linkedBalance': linkedBalance,
          'totalTickets': totalTickets,
          'playableTickets': playableTickets,
          'lastUpdated': FieldValue.serverTimestamp(),
        }, SetOptions(merge: true));
      } catch (e) {
        print("Failed to sync to backend: $e");
      }

      FocusScope.of(context).unfocus(); 
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Bought $qty tickets!"), backgroundColor: Colors.green[700]));
    }
  }
